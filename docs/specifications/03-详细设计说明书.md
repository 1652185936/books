...
| recognition_error_code | TEXT | NULL | 识别失败码，取值集见下 |
...
该列的 `CHECK IN (...)` 形式化约束在 D4 数据库约束批次随其它枚举一并补齐。
...
- 两阶段：先在一个数据库事务内按 BR-010 用户选择处理（保留或一并删除 `exercise_attempts`、`wrong_items`、`notes`、`highlights`、`review_cards`、`reading_progress`），再删除文件仓 `books/<bookId>`、`handwriting/<bookId>`、`notes/<bookId>` 与对应 `derived/`；文件删除失败可重试，不阻塞已提交的 DB 事务。
