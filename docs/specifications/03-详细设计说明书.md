...
- 失败区分**可重试**（IMP-013 存储不足、IMP-016 提交失败等环境类）与**终态拒绝**（IMP-002 路径穿越、IMP-009 哈希不符等内容类，不可重试）。
...
多空题整题结果由各空 `BlankGradeResult` 聚合（04 契约 §9）：全部 `correct` → `correct`；存在 `incorrect` → `incorrect`；否则 `partial`。整题 `score` 为各空 `score` 算术平均；任一空 `recognition-failed` 时整题不落最终 `grade`（见 §4.2）。
