...
- [x] P0-6 路由骨架 @pending — `utils/Routes.ets`（10 路由名 + 8 参数接口 + §6.13 三归一化纯函数 + ROUTE_TABLE）、`resources/base/profile/route_map.json` 系统路由表（9 NavDestination）+ module.json5 `routerMap`、`Index.ets` 改 Navigation 容器根页、9 占位页 + `BookshelfPage` + `RoutePlaceholder` 复用组件、三语补 11 条路由标题/占位文案；交叉一致性脚本 + `tsc --strict` + 归一化 13 例单测全 PASS（缺 SDK 未编译）。
