...
- [x] P1-2 内置 JSON Schema 校验器 @PENDING — `services/import/` 新增 `JsonSchemaValidator.ets`（draft 2020-12 子集纯函数引擎，违规只记结构化原因不含实例值）、`StudybookSchemaCatalog.ets`（文件→种类分类器，忽略包内 `schemas/`/`assets/` 封死信任边界 BR-002 + `SchemaProvider`/`InMemorySchemaProvider`）、`PackageSchemaValidator.ets`（解析 IMP-007 + 校验 IMP-008 编排），并把五个 Schema 复制进 `resources/rawfile/schemas/` 作内置可信副本；等价 Node 端口 40/40 PASS（逐关键字失败用例 + 非手写占位不强约束 R-063 + 关键字覆盖元测试）。`format` 按 2020-12 默认作注解不断言。未编译验证：缺 SDK，rawfile Schema 读取留待 P1-3 接线。
